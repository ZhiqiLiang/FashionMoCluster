...
Here is the crucial constants and their locations:
...
- `main`: The entry point of the script. It manages the entire fine-tuning process:
  ...
  - **Main Steps**:
    - **Preprocessing**: Dataset preparation, model initialization, optimizer setup.
    - **Training Loop**: Generates embeddings, computes clustering loss, updates the model.
    - **Early Stopping**: Monitors clustering performance and stops if no improvements are observed.
    - **Model Checkpoints**: Saves model states at intervals and when improvement occurs.
    - **Final Embeddings**: Saves the final embeddings and performs final spectral clustering.
...
1. **ConvNet only**: The baseline model without any advanced components such as residual connections or projection heads.
2. **ConvNet + ResConn**: This setup adds residual connections to the network, allowing the model to benefit from deeper architectures.
3. **ConvNet + ResConn + MLP**: Here, a projection head (MLP) is added to the model to help improve the representation power and aid in contrastive learning.
4. **ConvNet + ResConn + MLP + Advanced Augmentation**: In this setup, the model uses advanced augmentation strategies like noise, Gaussian blur, and Sobel filters, which help to enhance the robustness of the learned representations.
5. **ConvNet + ResConn + MLP + Advanced Augmentation + Cosine Annealing**: In addition to the previous setups, this configuration uses cosine annealing for the learning rate schedule, providing dynamic adjustment of the learning rate during training.
6. **ConvNet + ResConn + MLP + Advanced Augmentation + Cosine Annealing + Larger Model**: The model is scaled up to a larger size, testing the impact of increasing the number of layers and capacity.
7. **ConvNet + ResConn + MLP + Advanced Augmentation + Cosine Annealing + Larger Model + More Epochs**: This setup extends the training duration to test the effect of longer training on model performance.
